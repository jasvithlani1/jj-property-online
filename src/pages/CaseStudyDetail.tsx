...
export default function CaseStudyDetail() {
  ...
  return (
    <div className="w-full bg-white selection:bg-gold/20">
      ...
      {/* ── Editorial Hero ─────────────────────────────────────────────── */}
      <section className="relative w-full pt-36 md:pt-44 pb-12 overflow-hidden bg-gold/5">
        ...
        <div className="max-w-7xl mx-auto px-8 relative z-10">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <span className="px-4 py-1.5 rounded-full border border-gold/30 bg-gold/5 text-gold text-[10px] font-black uppercase tracking-widest">
                {study.tag || 'Case Study'}
              </span>
              <span className="text-muted/60 text-xs font-sans uppercase tracking-[0.2em] flex items-center gap-1.5">
                <MapPin className="w-3.5 h-3.5 text-gold" />
                {study.location}
              </span>
            </div>

            <h1 className="text-3xl md:text-6xl font-serif text-[#011122] leading-tight mb-4 max-w-4xl">
              {study.title}
            </h1>
            
            <p className="text-lg font-sans text-gold font-bold uppercase tracking-widest">{study.resultText}</p>
          </motion.div>
        </div>
      </section>

      {/* ── Featured Image ─────────────────────────────────────────────── */}
      <section className="relative px-4 md:px-8 -mt-6 mb-8">
        <motion.div
          initial={{ opacity: 0, scale: 0.98 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 1, delay: 0.2 }}
          className="max-w-6xl mx-auto h-[45vh] md:h-[60vh] rounded-[2.5rem] overflow-hidden shadow-xl border border-black/5"
        >
      ...
      <section className="py-6 px-8 bg-white">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-16">
          ...
          <div className="lg:col-span-7 flex flex-col gap-10">
            ...
            {study.client && (
              <motion.div
                ...
                className="space-y-3"
              >
                <h2 className="text-xs font-black uppercase tracking-[0.25em] text-gold">01 . Background</h2>
                <div className="text-base text-muted/95 font-sans leading-relaxed space-y-4 max-w-none">
                  {study.client.split('\n\n').map((p, i) => <p key={i}>{p}</p>)}
                </div>
              </motion.div>
            )}
            ...
            {study.challenge && (
              ...
              >
                <h2 className="text-xs font-black uppercase tracking-[0.25em] text-gold">02 . The Property Brief</h2>
                <div className="text-base text-muted/95 font-sans leading-relaxed space-y-4 max-w-none">
            ...
            {study.strategy && (
              <motion.div
                ...
                className="space-y-3 pt-2"
              >
                <h2 className="text-xs font-black uppercase tracking-[0.25em] text-gold">03 . Why This Property Made Sense</h2>
                <div className="text-base text-muted/95 font-sans leading-relaxed space-y-4 max-w-none">
            ...
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              className="p-8 md:p-10 rounded-[2.5rem] border border-gold/15 bg-gold/5 relative overflow-hidden"
            >
              <h3 className="text-base font-serif text-[#011122] mb-4">Strategic Advantages Delivered</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
                {[
                  ...
                  <div key={i} className="space-y-1">
                    <h4 className="font-serif text-[#011122] font-semibold text-sm">{item.title}</h4>
          ...
          <aside className="lg:col-span-5 flex flex-col gap-8">
            ...
            >
              <div className="border border-gold/15 bg-white rounded-[2.5rem] p-8 md:p-10 shadow-lg relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-gold/5 blur-[50px] rounded-full pointer-events-none" />
                <h3 className="text-xs font-black uppercase tracking-[0.25em] text-[#011122] mb-6 pb-4 border-b border-gold/10">Property Acquisition Details</h3>
                
                <div className="space-y-4">
                  {study.propertyDetails ? (
                    study.propertyDetails.map((detail, idx) => (
                      <div key={idx} className="flex justify-between items-center text-xs py-2 border-b border-black/5 last:border-0">
                        <span className="font-bold text-[#011122] uppercase tracking-wider text-[10px] opacity-60 shrink-0">{detail.label}</span>
                        <span className="font-sans font-medium text-[#011122] text-right ml-4">{detail.value}</span>
                      </div>
                    ))
                  ) : (
                    <>
                      <div className="flex justify-between items-center text-xs py-2 border-b border-black/5">
                        <span className="font-bold text-[#011122] uppercase tracking-wider text-[10px] opacity-60">Location</span>
                        <span className="font-sans font-medium text-[#011122] text-right ml-4">{study.location}</span>
                      </div>
                      {study.stats?.map((stat, idx) => (
                        <div key={idx} className="flex justify-between items-center text-xs py-2 border-b border-black/5 last:border-0">
                          <span className="font-bold text-[#011122] uppercase tracking-wider text-[10px] opacity-60 shrink-0">{stat.label}</span>
                          <span className="font-sans font-medium text-[#011122] text-right ml-4">{stat.value}</span>
                        </div>
                      ))}
                    </>
                  )}
              ...
              {/* Client Quote */}
              {study.shortQuote && (
                <div className="p-8 rounded-[2.5rem] bg-neutral-50 border border-black/5 relative shadow-sm">
                  <Quote className="w-6 h-6 text-gold/30 mb-3" />
                  <p className="text-xs font-sans italic text-muted leading-relaxed mb-4">"{study.shortQuote}"</p>
                  <p className="text-[9px] font-black uppercase tracking-widest text-[#011122]">— Verified Client Result</p>
                </div>
              )}
            </motion.div>
          </aside>
      ...
      {study.gallery && study.gallery.length > 0 && (
        <section className="py-8 px-8 bg-neutral-50 border-t border-b border-black/5">
          <div className="max-w-6xl mx-auto">
            <h2 className="text-xs font-black uppercase tracking-[0.4em] text-gold mb-8 text-center">Acquisition Gallery</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {study.gallery.map((img: any, idx: number) => (
                <motion.div
                  ...
                  className="rounded-2xl overflow-hidden aspect-square md:aspect-video relative group border border-black/5 shadow-md"
                >
      ...
      {/* ── Next Stories ────────────────────────────────────────────────── */}
      <section className="py-12 px-8 bg-white border-t border-black/5">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-xs font-black uppercase tracking-[0.4em] text-gold mb-8 text-center">Continue Exploring</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {otherStudies.map((other) => (
              <Link
                key={other._id}
                href={`/case-studies/${other.slug.current}`}
                className="group flex flex-col md:flex-row gap-6 p-8 rounded-[2.5rem] bg-neutral-50 border border-transparent hover:border-gold/20 hover:bg-white hover:shadow-xl transition-all duration-500"
              >
                <div className="w-full md:w-36 h-36 rounded-2xl overflow-hidden shrink-0 shadow-md">
                  <img 
                ...
                <div className="flex flex-col justify-center">
                  <span className="text-[9px] font-black uppercase tracking-widest text-gold mb-2">{other.tag || 'Acquisition'}</span>
                  <h3 className="text-xl font-serif text-[#011122] mb-2 group-hover:text-gold transition-colors line-clamp-2">{other.title}</h3>
